...
describe('Task 24 team, support, profile, affiliate and reseller contracts', () => {
  ...
  it('opens support tickets and lets the owner continue the conversation safely', () => {
    ...
    expect(detail).toContain("fromYou ? 'justify-end' : 'justify-start'")
    expect(detail).toContain("fromYou ? 'You' : 'Omnix support'")
    expect(detail).toContain('max-w-[min(88%,44rem)]')
  ...
  it('keeps affiliate sign-up, referral copy, anti-fraud states, and a bounded credit ledger', () => {
    const page = read(PAGES.affiliate)
    const client = read(PAGES.affiliateClient)
    ...
    expect(page).toContain('eq(affiliateCredits.affiliateId, aff.id)')
    expect(page).toContain('ilike(affiliateCredits.paymentId')
    expect(page).toContain('count()')
    expect(page).toContain('.limit(PAGE_SIZE)')
    expect(page).toContain('.offset(')
    expect(client).toContain('ListSearch')
    expect(client).toContain('ListPagination')
