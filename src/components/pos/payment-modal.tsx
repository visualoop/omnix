...
export function PaymentModal({ open, onClose }: Props) {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      if (saleSnapshot.sourceType === "salon_package" && saleSnapshot.sourceId) {
        const { finalizeSalonPackageSale } = await import("@/services/salon");
        await finalizeSalonPackageSale(saleSnapshot.sourceId, saleSnapshot.customerId, saleId);
      }
