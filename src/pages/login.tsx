      ...
      <div className="w-full max-w-[360px] space-y-6">
        <div className="text-center space-y-2">
          <div className="inline-flex items-center justify-center">
            {isModule ? (
              <ModuleLogo moduleId={moduleId} size={56} rounded />
            ) : (
              <OmnixLogo size={56} />
            )}
          </div>
        ...
        <form onSubmit={showReset ? handleResetPassword : handleLogin} className="space-y-3">
          ...
          {showReset ? (
            <div className="flex gap-2">
              <Button type="button" variant="outline" className="flex-1" onClick={() => { setShowReset(false); setError(""); }}>
            ...
            <>
              ...
              <button type="button" onClick={handleOpenReset} className="block w-full text-center text-xs text-muted-foreground hover:text-foreground transition-colors py-1">
                Forgot password?
              </button>
            </>
          )}
