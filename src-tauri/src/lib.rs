...
fn report_fatal(err: &str) {
    let log_path = std::env::var("LOCALAPPDATA")
        .ok()
        .map(|p| std::path::PathBuf::from(p).join("sokoos").join("crash.log"));
    ...
    #[cfg(windows)]
    {
        ...
        let title: Vec<u16> = OsStr::new("SokoOS — Startup Error")
            .encode_wide()
            .chain(std::iter::once(0))
            .collect();
        let body_str = format!(
            "{}\n\nA crash log was saved to:\n%LOCALAPPDATA%\\sokoos\\crash.log\n\nPlease send that file to support.",
            err,
        );
...
/// tries to open it. The plugin opens at $APPDATA/<identifier>/sokoos.db
