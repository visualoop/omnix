...
class OmnixMobilePlugin(private val activity: Activity) : Plugin(activity) {
    ...
    fun scannerAvailability(invoke: Invoke) {
        invoke.resolve(availability("unavailable", "The bundled scanner capture surface is not enabled"))
    }
    ...
    fun scannerRequestPermission(invoke: Invoke) {
        if (permissionState(Manifest.permission.CAMERA) != "granted") {
            ActivityCompat.requestPermissions(activity, arrayOf(Manifest.permission.CAMERA), 4101)
        }
        invoke.resolveObject(permissionState(Manifest.permission.CAMERA))
    }

    @Command
    fun scannerScan(invoke: Invoke) = invoke.resolve(null)

    @Command
    fun scannerCancel(invoke: Invoke) = invoke.resolve(empty())
    ...
    fun apkUpdateAvailability(invoke: Invoke) {
        invoke.resolve(availability("unavailable", "This build does not expose direct APK updates"))
    }

    private fun idleUpdate() = JSObject()
        .put("state", "idle")
        .put("releaseId", JSONObject.NULL)
        .put("downloadedBytes", 0)
        .put("totalBytes", 0)
        .put("errorCode", JSONObject.NULL)

    @Command
    fun apkUpdateStatus(invoke: Invoke) = invoke.resolve(idleUpdate())

    @Command
    fun apkUpdateStage(invoke: Invoke) = invoke.reject("Direct APK updates are unavailable")

    @Command
    fun apkUpdateInstall(invoke: Invoke) = invoke.reject("Direct APK updates are unavailable")

    @Command
    fun apkUpdateCancel(invoke: Invoke) = invoke.resolve(empty())
