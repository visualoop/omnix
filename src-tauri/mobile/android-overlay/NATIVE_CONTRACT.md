...
Use only `com.wireguard.android:tunnel:1.0.20260102`. Call `VpnService.prepare`; denial is not an app failure. Resolve `enrollmentId` to an account-scoped token in Keystore storage, generate/import the private key only in native memory, and pass it directly to WireGuard's `Config`/`GoBackend`. React receives public status only. Bind the tunnel to the authenticated account and explicit assigned branch; never infer branch from device state. Stop and zero temporary key/config buffers on account switch, revoke, sign-out, or plugin unload. No generic tunnel config, arbitrary endpoint, shell, SQL, HTTP, or private-key command is exposed.
