...
2. Copy `OmnixMobilePlugin.kt`, `OmnixMeshService.kt`, `BarcodeCaptureSession.kt`, and `DirectApkUpdater.kt` into `src-tauri/gen/android/app/src/main/java/co/ke/omnix/app/mobile/`, and copy `MainActivity.kt` into the parent app package. These files implement `NATIVE_CONTRACT.md`; `scripts/verify-android-overlay.mjs --generated` compares every mirror byte-for-byte. The Rust bridge is `src-tauri/src/mobile/mod.rs` and registers `co.ke.omnix.app.mobile.OmnixMobilePlugin`.
